-- Formula app password authentication is the login proof for event access.
-- Keep p_email_verified for backwards-compatible RPC calls, but do not require
-- inbox verification before linking an eligible ticket to its Firebase UID.
...
-- SECURITY DEFINER remains intentionally unavailable to ordinary signed-in
-- database users. The backend reaches it through the secret-gated anon bridge.
