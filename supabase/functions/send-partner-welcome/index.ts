...
function buildPartnerWelcomeEmailHtml(name: string | null, tier: string, sessionId: string): string {
  ...
  const onboardingUrl = `${WEBSITE_URL}/partner-welcome/${tier}?session_id=${sessionId}`;
...
serve(async (req) => {
  ...
  try {
    ...
    const { email, name, tier, sessionId } = await req.json();

    if (!email || !tier || !sessionId) {
      return new Response(
        JSON.stringify({ error: "Missing required fields: email, tier, sessionId" }),
    ...
    console.log("Partner welcome email sent to", email, "tier:", tier);
    ...
    console.error("Send partner welcome failed:", error);
    return new Response(
